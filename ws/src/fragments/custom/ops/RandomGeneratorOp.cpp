...
void
RandomGeneratorOp::setup(holoscan::OperatorSpec &spec) {
  HOLOSCAN_LOG_INFO("[RandomGeneratorOp::setup] Setting up...");
  spec.output<float>("out");
  spec.param(_minRange,
             "minRange",
             "Parameter 1",
             "A parameter for the custom bin",
             1.0f);
  spec.param(_maxRange,
             "maxRange",
             "Parameter 1",
             "A parameter for the custom bin",
             1.0f);
...
void
RandomGeneratorOp::compute(holoscan::InputContext     &in_output,
  ...
  HOLOSCAN_LOG_DEBUG("[RandomGeneratorOp::compute] Processed data: {}",
                     processed_data);

  // Send the processed data to the next operator in the pipeline
  out_output.emit(processed_data);
  HOLOSCAN_LOG_INFO("[RandomGeneratorOp::compute] Emitting processed data: {}",
                    processed_data);
}
